...
func getGtihubOAuthURL() (*oauth2.Config, string) {
	options := createClientOptions("github", "http://localhost:8080/callback/github")

	github_config = &oauth2.Config{
		ClientID:     options.getID(),
		ClientSecret: options.getSecret(),
		RedirectURL:  options.getRedirctURL(),
		//授权范围
		Scopes: []string{
			"user",
			"repo",
...
func GithubOAuthLogin(ctx *gin.Context) {
	config, state := getGtihubOAuthURL()
...
func GithubCallBack(ctx *gin.Context) {
	...
	client := github_config.Client(ctx.Request.Context(), token)
